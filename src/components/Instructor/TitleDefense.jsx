        ...
        panelSelect.addEventListener("change", () => {
          ...
          if (selectedId && !selectedPanelists.includes(selectedId)) {
            if (selectedPanelists.length < 3) {
              ...
              Swal.showValidationMessage('Maximum of 3 panelists can be selected.');
            }
          }
          panelSelect.value = "";
        });
  ...
  const handleUpdate = async (id) => {
    ...
    MySwal.fire({
          ...
          <div style="flex: 1;">
            ...
            <select id="panelSelect" class="form-select" style="border-radius: 8px; height: 42px;">
              ...
              ${advisers.map((a) => {
                ...
                const isSelected = selectedPanelists.includes(a.id);
                const disabledText = isDisabled ? " (Team Adviser)" : "";
                return `<option value="${a.id}" ${isSelected ? 'selected' : ''} ${isDisabled ? 'disabled' : ''}>
                          ${a.last_name}, ${a.first_name}${disabledText}
                        </option>`;
              }).join("")}
            </select>
          </div>
      ...
      didOpen: () => {
        ...
        // Add event listener for removing panelists
